...
# Prefijos al INICIO del Order ID
MP_PREFIX = {
    "DFT": "Dafiti",
    "GVL": "Agaval",
    "VPC": "Puntos Colombia",
...
MP_SUFFIX = MP_PREFIX  # alias compatibilidad

# Estados reales confirmados en la cuenta VTEX (5 estados)
STATUS_LABEL = {
    "invoiced":           "Facturado",
    "checking-invoice":   "Verificando factura",
    "handling":           "Preparando",
    "payment-approved":   "Pago aprobado",
    "ready-for-handling": "Listo para preparación",
}

VTEX_STATUSES = "invoiced,checking-invoice,handling,payment-approved,ready-for-handling"

def get_marketplace(order_id):
    """Lee el PREFIJO del Order ID. Ejemplo: DFT-5229258-284912359 -> Dafiti"""
    try:
        prefix = str(order_id).split("-")[0].upper().strip()
        return MP_PREFIX.get(prefix, None)
    except Exception:
        return None
...
def fetch_orders(date_from_str, date_to_str):
    """
    Trae ordenes en bloques de 3 dias (mas seguro que 7 para evitar
    el limite de 30 paginas en cuentas con alto volumen como esta).
    """
    ...
    while current <= date_to:
        chunk_end = min(current + timedelta(days=2), date_to)  # bloques de 3 dias
        ...
        while True:
            try:
                ...
                if page >= total_pages:
                    break
                if page >= 29:
                    pages_hit_limit.append(f"{f_from[:10]} a {f_to[:10]}")
                    break
                page += 1
                time.sleep(0.3)
    ...
    if pages_hit_limit:
        st.warning(
            f"⚠️ Algunos bloques de fechas tienen tanto volumen que alcanzaron "
            f"el límite de páginas de VTEX: {', '.join(pages_hit_limit)}. "
            f"Considera consultar rangos de fechas más cortos para esos días."
        )
...
def parse_orders(raw_orders, enrich_sample=300):
    rows    = []
    ...
    for idx, o in enumerate(raw_orders):
        try:
            ...
            # Si no hay totals.Items, calcular desde items o usar total
            if gmv_val == 0:
                if item_rows:
                    gmv_val = sum(ir["valor_total"] for ir in item_rows)
                elif total_val > 0:
